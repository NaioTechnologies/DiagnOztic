	...
	public TriangleTrame(byte[] data) {
		...
		if (data.length == 6 + nbrTriangles * 4 * 3 * 3) {
			for (int i = 0; i < nbrTriangles; i++) {
				...
				byte[] point3d_x_3 = new byte[] { data[offset + 27],
						data[offset + 26], data[offset + 25], data[offset+24] };
				byte[] point3d_y_3 = new byte[] { data[offset + 31],
						data[offset + 30], data[offset + 29], data[offset + 28] };
				byte[] point3d_z_3 = new byte[] { data[offset + 35],
						data[offset + 34], data[offset + 33], data[offset + 32] };
